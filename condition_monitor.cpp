...
#include "ui_condition_monitor.h"

condition_monitor::condition_monitor(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::condition_monitor)
{
    ui->setupUi(this);
}

void condition_monitor::paintEvent(QPaintEvent *){
    //生成Qlist数据
    getInput();
    //设置绘图参数
    ...
    for(int i=0;i<getPcNum;i++){
        ...
        //设置画笔颜色
        if(lineColor==0) {painter.setPen("red");}
        else if (lineColor==1) {painter.setPen("yellow");}
        else if (lineColor==2) {painter.setPen("green");}
        painter.rotate(360/getPcNum*i);
        painter.drawLine(0, 0, lineLength, 0);
        painter.translate(lineLength, 0);
        painter.rotate(-360/getPcNum*i);
...
void condition_monitor::getInput(){
